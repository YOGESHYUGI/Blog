...
function Blog() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = (await axios.get("/blogs/getallblogs")).data
        setblogs(data)
      } catch (error) {
        console.log(error)
      }
  ...
  return (
    <div className='blog' > 
      ...
      {side ? <Sidebar /> : ""}
      <div className={`blog1 ${side ? "blog1-active":""}`}>
        {blogs.map((x) => {
          return (
            <div key={x._id} className="card">
              ...
              <div className='cardinside'>
                <img src={x.images} className='cardimg' alt='image' />
                <p className='cardpara'>{x.Desc}</p>
              </div>
              <a href="#" className='carda'>read more</a>
            </div>)
        })}
      </div>
    </div>
